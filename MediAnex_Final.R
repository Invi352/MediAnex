...
#if (!require('bslib')) install.packages('bslib');library('bslib')
#if (!require('bs4Dash')) install.packages('bs4Dash'); library('bs4Dash')
...
Test_data <- read.csv(file = "Final_Test/TestData.csv", header = TRUE, sep = ",")
Train_Data <- read.csv(file = "Final_Test/Train_RA.csv", header = TRUE, sep = ",")
Train_RML <- read.csv(file = "Final_Test/Train_RA2.csv", header = TRUE, sep = ",")
Network_Data <- read.csv(file = "Final_Test/Net_Data.csv", header = TRUE, sep = ",")
    ...
    introjs(session, options = list(
      ...
      steps = list(
        list(element = "#multivar_stats_nav",
             intro = "Welcome to the Mediator Analysis Nexus, or MediAnex for sort. Through this and various others tutorials
             in other pages you can learn to use and navigate this application. This is the primary navigation bar which will allow you to move 
             between different section and perform different analysis. ",
             position = "bottom"),
        list(element = "#multivar_stats_nav",
             intro = "To move to a different section all you need to do is simply press the Multivariate Sttistics 
             or Machine Learning Models panels and then select one of the relevant options available.",
             position = "bottom"),
        list(element = "#PCA",
             intro = "This is the input field where you have the options to upload a specific file. To do so simply bress the browse button and then upload a relevant 
             file from your files. Most sections of this application accept comma-separated, semicolon-separated or tab-separated files (csv, tsv or tsv respectively).",
             position = "right"),
        list(element = "#PCA",
             ...
             To do so, simply press on the download exmample file buttona and this will allow you to download a csv file so you become familiar with the format accepted. 
        ...
        list(element = "#PCA_col",
             ...
             names on your graph, you can do so by un-ticking the box which says sample names 
    ...
    introjs(session, options = list(
      ...
      steps = list(
        ...
        list(element = "#normality",
             ...
             position = "right"),
        list(element = "#normality",
             intro = "The application automatically reads the uploaded file and automatically updates the drop-down menus
